...
/**
  ...
  * @param  None
  * @retval None
	* @用到的寄存器 时钟控制寄存器(RCC_CR)、时钟配置寄存器(RCC_CFGR)、时钟中断寄存器(RCC_CIR)、
	* APB2外设复位寄存器(RCC_APB2RSTR)、APB1外设复位寄存器(RCC_APB1RSTR)、AHB外设时钟使能寄存器(RCC_AHBENR)、
	* APB2外设时钟使能寄存器(RCC_APB2ENR)、APB1外设时钟使能寄存器(RCC_APB1ENR)、备份域控制寄存器(RCC_BDRC)、
	* 控制/状态寄存器(RCC_CSR)、AHB外设时钟复位寄存器(RCC_AHBRSTR)、时钟配置寄存器2(RCC_CFGR2)
  */
void SystemInit (void)
{
  /*!< Set MSION bit */
  RCC->CR |= (uint32_t)0x00000100;//设置时钟控制寄存器

  /*!< Reset SW[1:0], HPRE[3:0], PPRE1[2:0], PPRE2[2:0], MCOSEL[2:0] and MCOPRE[2:0] bits */
	RCC->CFGR &= (uint32_t)0x883FC00C;//设置时钟配置寄存器
  ...
  RCC->CIR = 0x00000000;//时钟中断寄存器，关闭所有中断
...
static void SetSysClock(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    PWR->CR = PWR_CR_VOS_1;//Vcore = 1.5V
...
//                                         RCC_CFGR_PLLDIV));
//     RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLMUL8 | RCC_CFGR_PLLDIV2);//HSE*8/2=7372800*4
...
//     RCC->CFGR |= (uint32_t)RCC_CFGR_SW_PLL;
	  ...
	  RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));
	  RCC->CFGR |= (uint32_t)RCC_CFGR_SW_HSE;//选择HSE为系统时钟源
	  RCC->CFGR |= (uint32_t)RCC_CFGR_HPRE_DIV1;//SYSCLK 不分频  7.3728M*4
