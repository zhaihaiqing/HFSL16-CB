...
//定义通道1、2、3、4增益
int32_t temp_data;                  //定义温度缓存数据，用于暂存采集到的温度数据
...
uint32_t FreqValue;					//定义频率缓存数据，用于暂存采集到的频率数据             
...
char sample_stress_channel(unsigned char channel)//参数1234
{
	...
	if(channel)
	{
		...
		freq32 = ((uint64_t)SystemCoreClock  * 1000)/ ival;
#ifdef	Debug_EN
		printf("Chan %d after_filter:k= %d db= %d err_cnt= %d freq= %.3f\t%d\r\n",channel, ival, db,err_cnt, (double)freq32 /1000,(uint32_t)freq32);
#endif
		FreqValue=freq32;
...
int32_t get_temp_analog(char i,unsigned char Channel)//参数1234
{
		...
		temp_data = (int32_t)(temp_sum * 200);	
#ifdef	Debug_EN
		printf("模拟温度%d:%d\r\n", Channel,temp_data);
#endif
     return SUCCESS;
...
void Auto_Smaple(void)
{
	...
	{			
		...
		if(temp_data<-7500)				//失败执行数字采样，成功该位置1
		{	
